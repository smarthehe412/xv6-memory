...
#define GET_REST(p)    (GET(p) & 0x7)
#define GET_ALLOC(p)	 (GET(p) & 0x1)
#define GET_PREV_ALC(p)  ((GET(p) & 0x2) >> 1)
#define GET_PREV_MINI(p) ((GET(p) & 0x4) >> 2)
...
void insert(void *ptr) {
  //if miniblock then do nothing
  if(GET_SIZE(HDRP(ptr)) < LEAST_SIZE) return;
...
int mm_init(void) {
  ...
  //Prologue: 20/1, pre(2), suc(2)
  PUT(free_list_headp + 1 * WSIZE, PACK(20, 1));
  ...
  //Epilogue: 0/3, pre(2)
  PUT(HDRP(free_list_tailp), PACK(0, 3));				
...
void *new_alloc(int wcnt) {
  ...
  if(size == MINISIZE) {
    if(DEBUG) printf("alloc mini in\n");
    PUT(HDRP(p), PACK(size, GET_REST(HDRP(p)) & ~1));
    free_list_tailp = NEXT_BLKP(p);
    SET_SUCC(GET_PRED(p), free_list_tailp);
    PUT(FTRP(p), PACK(size, 0));

    //Epilogue
    PUT(HDRP(free_list_tailp), PACK(0, 5));
    SET_PRED(free_list_tailp, GET_PRED(p));
...
void *merge(void* p) {
  ...
  void *prev_ptr;
  if(GET_PREV_ALC(HDRP(p))) prev_ptr = 0;
  else if(GET_PREV_MINI(HDRP(p))) prev_ptr = p - MINISIZE;
  else prev_ptr = PREV_BLKP(p);
  void *next_ptr = NEXT_BLKP(p);

  uint prev_spare = prev_ptr && !GET_ALLOC(HDRP(prev_ptr)), next_spare = !GET_ALLOC(HDRP(next_ptr));
  ...
  if(prev_spare && next_spare) {
    ...
    insert(prev_ptr);
    PUT(HDRP(prev_ptr), PACK(siz1 + siz2 + siz3, GET_REST(HDRP(prev_ptr))));
    PUT(FTRP(prev_ptr), PACK(siz1 + siz2 + siz3, 0));
    PUT(HDRP(NEXT_BLKP(prev_ptr)), PACK(GET_SIZE(HDRP(NEXT_BLKP(prev_ptr))), GET_ALLOC(HDRP(NEXT_BLKP(prev_ptr)))));
    return prev_ptr;
  } else if (prev_spare && !next_spare) {
    int siz1 = GET_SIZE(HDRP(prev_ptr)), siz2 = GET_SIZE(HDRP(p));
    delete(prev_ptr), delete(p);
    insert(prev_ptr);
    PUT(HDRP(prev_ptr), PACK(siz1 + siz2, GET_REST(HDRP(prev_ptr))));
    ...
    insert(p);
    PUT(HDRP(p), PACK(siz1 + siz2, GET_REST(HDRP(p))));
    PUT(FTRP(p), PACK(siz1 + siz2, 0));
    PUT(HDRP(NEXT_BLKP(p)), PACK(GET_SIZE(HDRP(NEXT_BLKP(p))), GET_ALLOC(HDRP(NEXT_BLKP(p)))));
    return p;
  } else {
    return p;
  }
...
void place(void *p, uint size) {
  ...
  if(tsize - size >= LEAST_SIZE) {
    ...
    PUT(HDRP(np), PACK(tsize - size, 2));
    SET_PRED(np, GET_PRED(p));
    SET_SUCC(np, GET_SUCC(p));
    SET_PRED(GET_SUCC(p), np);
    SET_SUCC(GET_PRED(p), np);
    PUT(FTRP(np), PACK(tsize - size, 0));
  } else if(tsize - size >= MINISIZE) {
    PUT(HDRP(p), PACK(tsize + size - MINISIZE, 1));
    delete((char*)p);
    PUT(FTRP(p), PACK(tsize + size - MINISIZE, 1));
    void *np = NEXT_BLKP(p);
    if(DEBUG) printf("place p=%d np=%d pred=%d succ=%d\n",p,np,GET_PRED(p),GET_SUCC(p));
    PUT(HDRP(np), PACK(MINISIZE, 2));
    PUT(FTRP(np), PACK(MINISIZE, 0));
    PUT(HDRP(NEXT_BLKP(np)), PACK(GET_SIZE(HDRP(NEXT_BLKP(np))), 4 | GET_ALLOC(HDRP(NEXT_BLKP(np)))));
  } else {
    PUT(HDRP(p), PACK(tsize, 1));
    delete((char*)p);
    PUT(FTRP(p), PACK(tsize, 1));
    PUT(HDRP(NEXT_BLKP(p)), PACK(GET_SIZE(HDRP(NEXT_BLKP(p))), 2 | GET_ALLOC(HDRP(NEXT_BLKP(p)))));
  }
}
void *mm_malloc(uint size) {
  if(DEBUG) printf("malloc in\n"); 
  int newsize = MAX(ALIGN(size + WSIZE), MINISIZE);
  printf("%d\n", newsize);
  void *p = find(newsize);
  if(DEBUG) printf("finded p=%d\n", p);
  if(p == 0) {
...
void mm_free(void *ptr) {
  ...
  PUT(HDRP(ptr), PACK(GET_SIZE(HDRP(ptr)), GET_REST(HDRP(ptr)) & ~1));
  insert((char*)ptr);
  PUT(FTRP(ptr), PACK(GET_SIZE(HDRP(ptr)), 0));
  if(GET_SIZE(HDRP(ptr)) == MINISIZE) {
    PUT(HDRP(NEXT_BLKP(ptr)), PACK(GET_SIZE(HDRP(NEXT_BLKP(ptr))), 4 | GET_ALLOC(HDRP(NEXT_BLKP(ptr)))));
  } else {
...
void *mm_realloc(void *ptr, uint size) {
  ...
  uint newsize = MAX(ALIGN(size + WSIZE), MINISIZE);
  ...
  if (newsize == tsize) {
    ...
    if(!GET_ALLOC(HDRP(np)) && nsize + tsize >= newsize) {
      if(nsize + tsize - newsize >= LEAST_SIZE) {
        PUT(HDRP(ptr), PACK(newsize, GET_REST(HDRP(ptr))));
        ...
        SET_PRED(tsuc, nsp);
        SET_SUCC(tpre, nsp);
        PUT(HDRP(nsp), PACK(nsize + tsize - newsize, 2));
        PUT(FTRP(nsp), PACK(nsize + tsize - newsize, 0));

        //PUT(FTRP(ptr), PACK(newsize, 1));
      } else if (nsize + tsize - newsize >= MINISIZE) {
        PUT(HDRP(ptr), PACK(nsize + tsize - MINISIZE, GET_REST(HDRP(ptr))));
        delete(np);

        //wait for the update of free list
        void *nsp = NEXT_BLKP(ptr);
        PUT(HDRP(nsp), PACK(MINISIZE, 2));
        PUT(FTRP(nsp), PACK(MINISIZE, 0));

        //PUT(FTRP(ptr), PACK(nsize + tsize - MINISIZE, 1));
      } else {
    ...
    if(tsize - newsize >= LEAST_SIZE) {
      ...
      PUT(FTRP(ptr), PACK(newsize, 1));
      merge(nsp);
    } else if(tsize - newsize >= MINISIZE) {
      PUT(HDRP(ptr), PACK(newsize, 1));
